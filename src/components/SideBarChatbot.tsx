...
import ThemeToggle from "../components/ThemeToggleChatbot";
...
export default function Sidebar() {
  ...
  return (
    <aside className="group fixed top-0 left-0 h-full w-24 hover:w-64 bg-background shadow-md transition-all duration-300 z-50 overflow-hidden flex flex-col justify-between">
      {/* Top - Logo */}
      ...
      <nav className="flex flex-col space-y-1 px-2">
        <SidebarItem
          icon={<Home size={30} className="text-foreground" />}
          label="Dashboard"
          href="/dashboard"
        />
        <SidebarItem
          icon={<ClipboardList size={30} className="text-foreground" />}
          label="Orders"
          href="/orders"
        />
        <SidebarItem
          icon={<Users size={30} className="text-foreground" />}
          label="Staffs"
          href="/staff"
        />
        <ThemeToggle />
