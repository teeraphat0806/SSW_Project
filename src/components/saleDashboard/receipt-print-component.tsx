...
export function ReceiptPrintComponent({
  ...
  if (!receiptsData || receiptsData.length === 0) {
    return (
      <div className="max-w-[210mm] mx-auto bg-white text-[18px] p-6">
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white text-[18px]">
      {receiptsData.map((receiptData, receiptIndex) => {
        const isLastReceipt = receiptIndex === receiptsData.length - 1;
        const monthlyData =
          receiptData.type === "monthly"
            ? (receiptData.data as MonthlyPrintData)
            : null;
        const yearlyData =
          receiptData.type === "yearly"
            ? (receiptData.data as YearlyPrintData)
            : null;
        const customerInfo = receiptData.customerInfo;
        const customerName = receiptData.customerName;
        const year = receiptData.year;
                  ...
                  <>
                    {paginateRows(monthlyData.rows).map(
                      (pageRows, pageIndex, pages) => {
                        const isLastPageOfReceipt =
                          pageIndex === pages.length - 1 && isLastReceipt;
                        return (
                          <div
                            key={`${receiptIndex}-${pageIndex}`}
                          ...
                          >
                            ...
                            <div className="flex justify-between items-start mb-6">
                              ...
                              <div className="text-left text-[16px] leading-snug flex-1">
                                ...
                                {customerInfo?.taxId && (
                                  <p>
                                    เลขประจำตัวผู้เสียภาษี {customerInfo.taxId}
                                  </p>
                                )}
                              </div>
                    ...
                    {(() => {
                      ...
                      return Array.from({ length: totalPages }).map(
                        (_, pageNum) => {
                          ...
                          const groupedByMonth = pageRowIndices.reduce(
                            (acc, item) => {
                              if (!acc[item.monthIndex])
                                acc[item.monthIndex] = [];
                              acc[item.monthIndex].push(
                                yearlyData.months[item.monthIndex].rows[
                                  item.rowInMonth
                                ]
                              );
                              return acc;
                          ...
                          const isLastPageOfReceipt =
                            pageNum === totalPages - 1 && isLastReceipt;

                          return (
                            <div
                              key={`${receiptIndex}-yearly-${pageNum}`}
                              className="page-break p-6 flex flex-col min-h-screen"
                              style={{
                                pageBreakAfter:
                                  pageNum < totalPages - 1 || !isLastReceipt
