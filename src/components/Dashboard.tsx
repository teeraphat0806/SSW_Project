...
const Dashboard = ({ role }: DashboardProps) => {
  ...
  const getStatsForRole = () => {
    const stats = {
      clerk: [
        {
          label: "รายการปัจจุบัน",
          value: jobOrders.filter((jo) => jo.status === "pending").length,
          icon: FileText,
          color: "bg-blue-500",
        },
        {
          label: "พร้อมแจ้งชำระ",
          value: jobOrders.filter((jo) => jo.status === "ready").length,
          icon: DollarSign,
          color: "bg-red-500",
        },
        {
        ...
        {
          label: "รายการสำเร็จ",
          value: jobOrders.filter((jo) => jo.status === "completed").length,
          icon: CheckCircle,
          color: "bg-blue-500",
        },
      ],
      supervisor: [
        ...
        {
          label: "กำลังตัด",
          value: jobOrders.filter((jo) => jo.status === "cutting").length,
          icon: Scissors,
          color: "bg-red-500",
        },
      ...
      cutter: [
        ...
        {
          label: "รายการ กำลังตัด",
          value: 3,
          icon: Clock,
          color: "bg-red-500",
        },
        {
          label: "ตัดแล้วกี่ชิ้น",
          value: 127,
          icon: Package,
          color: "bg-blue-500",
        },
      ...
      delivery: [
        ...
        {
          label: "กำลังจัดส่ง",
          value: jobOrders.filter((jo) => jo.status === "shipped").length,
          icon: Truck,
          color: "bg-red-500",
        },
        {
      ...
      <div className="container mx-auto p-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground">
                ยินดีต้อนรับสู่ระบบจัดการงานตัดเหล็กของเรา!
                เลือกบทบาทของคุณเพื่อดูข้อมูลที่เกี่ยวข้อง
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Button
                ...
                onClick={() => router.push("/new-job-order")}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {getStatsForRole().map((stat, index) => (
            <Card
              key={index}
              className={`shadow-steel border-none hover:shadow-elevation transition-shadow ${stat.color}`}
            >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium">
                            {order.width}×{order.length}×{order.thickness}mm
                          </p>
                        </div>
                        {order.weight && (
                          <div>
                        ...
                        <div className="flex gap-2">
                          {currentRole === "clerk" && (
                            <>
                              <Button
                                size="sm"
                                className="bg-blue-500 text-white border-none hover:bg-blue-600 hover:scale-110 hover:cursor-pointer transition-all"
                                variant="outline"
                                onClick={() =>
                                  router.push(`/job-order/${order.id}`)
                                }
                              >
                                รายละเอียด
                              </Button>
                              <Button
                                size="sm"
                                className="border-1 border-black hover:cursor-pointer hover:scale-110 transition-all "
                              >
                                สรุปรายการ
                              </Button>
                            </>
                  ...
                  {[
                    {
                      status: "pending",
                      label: "รายการปัจจุบัน",
                      count: jobOrders.filter((jo) => jo.status === "pending")
                        .length,
                    },
                    {
                      status: "cutting",
                      label: "กำลังตัด",
                      count: jobOrders.filter((jo) => jo.status === "cutting")
                        .length,
                    },
                    {
                      status: "weighing",
                      label: "ชั่งน้ำหนัก",
                      count: jobOrders.filter((jo) => jo.status === "weighing")
                        .length,
                    },
                    {
                      status: "ready",
                      label: "พร้อมจัดส่งสินค้า",
                      count: jobOrders.filter((jo) => jo.status === "ready")
                        .length,
                    },
                    {
                      status: "shipped",
                      label: "กำลังจัดส่งสินค้า",
                      count: jobOrders.filter((jo) => jo.status === "shipped")
                        .length,
                    },
                    {
                      status: "completed",
                      label: "สำเร็จ",
                      count: jobOrders.filter((jo) => jo.status === "completed")
                        .length,
                    },
                  ].map((item) => (
                    <div
              ...
              <CardContent>
                <div className="space-y-2">
                  {currentRole === "clerk" && (
                    <>
                      <Button
                        className="w-full justify-start border-1 border-black hover:cursor-pointer hover:scale-110 transition-all"
                        onClick={() => router.push("/new-job-order")}
                      >
