...
export default function MonthSelectionModal({
  ...
  const handleSelectAllAvailable = () => {
    setSelectedMonths(Array.from(monthsWithAvailableData));
  };
  ...
  const handleConfirm = () => {
    ...
    const invoicesToAdd = invoicesByMonth.filter(
      (invoice) => !selectedInvoices.some((selected) => selected.id === invoice.id),
    );

    onConfirm(invoicesToAdd);
    setSelectedMonths([]);
    onOpenChange(false);
  };
